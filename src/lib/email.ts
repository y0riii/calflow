...
async function sendEmail(to: string, subject: string, html: string): Promise<void> {
  try {
    if (!process.env.AUTO_EMAIL || !process.env.AUTO_EMAIL_PASSWORD) {
      console.warn('⚠️ SMTP credentials missing. Email skipped for:', to);
      return;
    }
    await transporter.sendMail({
      from: `"Calendly Clone" <${process.env.AUTO_EMAIL}>`,
      to,
      subject,
      html,
    });
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <tr>
          <td style="background:${accentColor};padding:28px 36px;">
            <p style="margin:0;color:#ffffff;font-size:11px;letter-spacing:2px;text-transform:uppercase;font-weight:600;">Calendly Clone</p>
        ...
        <tr>
          <td style="padding:20px 36px;border-top:1px solid #e2e8f0;">
            <p style="margin:0;color:#94a3b8;font-size:11px;line-height:1.6;">
              This is an automated notification from Calendly Clone. Please do not reply to this email.
            </p>
          </td>
        </tr>
      </table>
...
export async function sendBookingCreatedEmails(booking: BookingDetails): Promise<void> {
  ...
  const guestHtml = baseTemplate(
    ...
    `<p style="margin:0 0 16px;color:#334155;font-size:15px;">Hi <strong>${booking.guestName}</strong>,</p>
     ...
     <p style="margin:0 0 12px;color:#475569;font-size:13px;font-weight:600;">Need to make changes?</p>
